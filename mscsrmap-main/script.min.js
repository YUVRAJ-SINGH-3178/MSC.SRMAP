...
if (cDot && cRing) {
  ...
  s.forEach((e) => {
    (e.addEventListener("mouseenter", () => {
      (cDot.classList.add("h"), cRing.classList.add("h"));
    }),
      e.addEventListener("mouseleave", () => {
        (cDot.classList.remove("h"), cRing.classList.remove("h"));
      }));
  });
}
...
if (hamburger && navMenu) {
  ...
  (hamburger.addEventListener("click", (e) => {
    ...
    document.addEventListener("click", (e) => {
      hamburger.contains(e.target) || navMenu.contains(e.target) || closeNavMenu();
    }),
  ...
  document.querySelectorAll('a[href^="#"]').forEach((e) => {
    e.addEventListener("click", function (e) {
      ...
      const t = this.getAttribute("href"),
        ...
        a = document.querySelector(".hamburger");
      if (
        (o &&
...
(window.addEventListener("scroll", () => {
  ...
  window.addEventListener("scroll", () => {
    const e = document.querySelector(".navbar"),
      t = document.querySelector(".background-overlay"),
      n = document.querySelector(".scroll-indicator"),
      o = 0.7 + 0.15 * Math.min(window.scrollY / window.innerHeight, 1);
    ((t.style.background = `rgba(0, 0, 0, ${o})`),
      window.scrollY > 100
        ? ((e.style.background = "rgba(15, 15, 20, 0.75)"),
          (e.style.backdropFilter = "blur(25px) saturate(150%)"),
          (e.style.boxShadow =
            "0 4px 24px rgba(0, 0, 0, 0.5), inset 0 1px 0 rgba(255, 255, 255, 0.08)"))
        : ((e.style.background = "rgba(10, 10, 15, 0.6)"),
          (e.style.backdropFilter = "blur(20px) saturate(140%)"),
          (e.style.boxShadow =
            "0 4px 24px rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05)")));
    const a = document.querySelectorAll("section[id]"),
      s = document.querySelectorAll(".nav-link"),
      r = document.querySelectorAll(".checkpoint-dot");
    let i = "";
    (a.forEach((e) => {
      const t = e.offsetTop;
      e.clientHeight;
      window.scrollY >= t - 150 && (i = e.getAttribute("id"));
    }),
      s.forEach((e) => {
        (e.classList.remove("active"),
          e.getAttribute("href") === "#" + i && e.classList.add("active"));
      }),
      r.forEach((e) => {
        (e.classList.remove("active"),
          e.getAttribute("data-section") === i && e.classList.add("active"));
      }),
      n &&
        ("contact" === i
      ...
      sections: [
        {
          ...
          content:
            "<p>Solutions for Smart India is a national-level hackathon designed to encourage students to innovate and build impactful technology solutions that improve everyday life for Indians and contribute to the nation's progress.</p>",
        },
        {
          title: "Event Details",
          content:
        ...
        },
        {
          ...
          content:
            "<p>Solve real-world problems in domains like Healthcare, Education, Agriculture, Smart Cities, and Sustainability. Detailed problem statements will be revealed on the event day.</p>",
        },
      ],
    },
  };
function openEventModal(e) {
  const t = eventData[e];
  if (!t) return;
  ((modalTitle.textContent = t.title), (modalDate.textContent = t.date));
  let n = "";
...
(closeModal &&
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const e = document.getElementById("hackmsc2-more-btn");
    e && e.addEventListener("click", () => openEventModal("hackmsc2"));
  }));
...
(document.addEventListener("DOMContentLoaded", () => {
  document
    .querySelectorAll(".event-card, .founder-card, .stat-item, .contact-item")
    .forEach((e) => {
      ((e.style.opacity = "0"),
  ...
  document.querySelectorAll(".register-btn").forEach((e) => {
...
function showStatus(e, t) {
  ((formStatus.textContent = e),
    (formStatus.className = `form-status ${t}`),
    "success" === t &&
      setTimeout(() => {
        ((formStatus.textContent = ""), (formStatus.className = "form-status"));
      }, 5e3));
}
function typeWriter(e, t, n = 100) {
  let o = 0;
  ((e.innerHTML = ""),
    (function a() {
      o < t.length && ((e.innerHTML += t.charAt(o)), o++, setTimeout(a, n));
    })());
}
(contactForm &&
  contactForm.addEventListener("submit", async (e) => {
    ...
    if (!t || !n || !o)
      return void showStatus("Please fill in all fields.", "error");
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n)) {
      ((submitBtn.disabled = !0),
        (submitBtn.textContent = "Sending..."),
        showStatus("Sending your message...", "info"));
      try {
        const e = new FormData(contactForm),
          t = await fetch("https://api.web3forms.com/submit", {
            method: "POST",
            body: e,
          }),
          n = await t.json();
        if (!n.success) throw new Error(n.message || "Failed to send email");
        (showStatus(
        ...
        ((submitBtn.disabled = !1), (submitBtn.textContent = "Send Message"));
      }
    } else showStatus("Please enter a valid email address.", "error");
  }),
  window.addEventListener("scroll", () => {
    const e = window.pageYOffset;
    document.querySelectorAll(".glow-circle").forEach((t, n) => {
      const o = 0.5 + 0.1 * n;
  ...
  document.querySelectorAll(".event-card, .founder-card").forEach((e) => {
    (e.addEventListener("mouseenter", () => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".page-transition").forEach((e) => {
      e.addEventListener("click", function (e) {
        e.preventDefault();
        const t = this.getAttribute("href");
...
((style.textContent =
  ...
  window.addEventListener("load", () => {
    ...
    e &&
      ...
      setTimeout(() => {
        e.play().catch((t) => {
          (console.log("Video autoplay prevented:", t),
            setTimeout(() => {
              e.play().catch((e) => console.log("Second attempt failed:", e));
            }, 500));
        });
      }, 1e3));
  }));
...
(window.addEventListener("load", () => {
  const e = document.getElementById("loadingScreen"),
    t = document.getElementById("loadingFact");
  if (t) {
    const e = techFacts[Math.floor(Math.random() * techFacts.length)];
    t.textContent = e;
  }
  setTimeout(() => {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const cassettes = document.querySelectorAll(".event-cassette");
  const isTouchDevice = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
  
  if (!isTouchDevice) {
    cassettes.forEach(card => {
      let tiltRAF = null;
      card.addEventListener('mousemove', (e) => {
        ...
        tiltRAF = requestAnimationFrame(() => {
          ...
          const pullX = normalX * 4;
          const pullY = normalY * 4;
          
          card.style.transform = `perspective(1000px) skewY(var(--scroll-skew, 0deg)) rotateX(${tiltX}deg) rotateY(${tiltY}deg) translateX(${pullX}px) translateY(${pullY}px) scale(1.02)`;
